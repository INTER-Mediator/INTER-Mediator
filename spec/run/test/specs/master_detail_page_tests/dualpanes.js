module.exports = (dualPage) => {
  ...
  describe('Master-Detail Page', () => {
    ...
    it('5-can move to next page of navigator, and checking', async () => {
      ...
      await dualPage.masterFieldPostalCodeFirst.waitForExist()
      await dualPage.masterFieldPrefFirst.waitForExist()
      await dualPage.masterFieldCityFirst.waitForExist()
      await dualPage.masterFieldTownFirst.waitForExist()
      expect(await dualPage.masterFieldPostalCode.length).toBe(100)
      expect(await dualPage.masterFieldPref.length).toBe(100)
      expect(await dualPage.masterFieldCity.length).toBe(100)
      expect(await dualPage.masterFieldTown.length).toBe(100)
      expect(await dualPage.masterFieldPostalCode[0]).toHaveValue('1006122')
      expect(await dualPage.masterFieldPref[0]).toHaveValue('東京都')
      expect(await dualPage.masterFieldCity[0]).toHaveValue('千代田区')
      expect(await dualPage.masterFieldTown[0]).toHaveValue('永田町山王パークタワー（２２階）')
      expect(await dualPage.detailFieldPostalCode).toHaveValue('1006122')
      expect(await dualPage.detailFieldPref).toHaveValue('東京都')
      expect(await dualPage.detailFieldCity).toHaveValue('千代田区')
      expect(await dualPage.detailFieldTown).toHaveValue('永田町山王パークタワー（２２階）')
    })
    it('6-can move to next page of navigator again, and checking', async () => {
      await dualPage.navigatorMoveButtonNext.click()
      browser.pause(waiting)
      ...
      expect(await dualPage.masterFieldCity.length).toBe(100)
      expect(await dualPage.masterFieldTown.length).toBe(100)
      expect(await dualPage.masterFieldPostalCode[0]).toHaveValue('1006407')
      expect(await dualPage.masterFieldPref[0]).toHaveValue('東京都')
      expect(await dualPage.masterFieldCity[0]).toHaveValue('千代田区')
      expect(await dualPage.masterFieldTown[0]).toHaveValue('丸の内東京ビルディング（７階）')
      expect(await dualPage.detailFieldPostalCode).toHaveValue('1006407')
      expect(await dualPage.detailFieldPref).toHaveValue('東京都')
      expect(await dualPage.detailFieldCity).toHaveValue('千代田区')
      expect(await dualPage.detailFieldTown).toHaveValue('丸の内東京ビルディング（７階）')
